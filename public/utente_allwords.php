...
$dbPath = 'mio_database.sqlite';

$db = new SQLite3($dbPath);

$db->exec("CREATE TABLE IF NOT EXISTS parole (
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['parola'])) {
    $parola = $_POST['parola'];
    $stmt = $db->prepare("INSERT INTO parole (valore) VALUES (:parola)");
    $stmt->bindValue(':parola', $parola, SQLITE3_TEXT);
    $stmt->execute();
}
